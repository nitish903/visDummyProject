...
const WorldMap = ({ onChangeCountry }) => {
  const svgRef = useRef();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/worldmap-data');
  ...
  }, [newCountry, onChangeCountry]);

  useEffect(() => {
    if (Object.keys(data).length === 0 || loading) return;
  ...
  }, [data, variable, newCountry, loading]);
